...
void VulkanExampleBase::postRenderLoop(){
...
void VulkanExampleBase::rotateCamera(float dx, float dy, float dz){
	rotation.x += dx * 4.0f;
	rotation.y += dy * 4.0f;
	rotation.z += dz * 4.0f;
//	camera.rotate(glm::vec3(dy * camera.rotationSpeed, -dx * camera.rotationSpeed, dz * camera.rotationSpeed));
	viewUpdated = true;
}
